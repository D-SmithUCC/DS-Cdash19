...
for line in datarows:
	...
	recoverradius = int(recover)*25
	
	if (int(recover) > 0):
		if (prov != ""):
			marker = "L.circle([" + lat + "," + lon + "],{color:'#3c9124',fillColor:'#42cc1b',fillOpacity:0.5,radius:" + str(recoverradius) + "}).addTo(map).bindPopup('" + prov.replace("'", "") + "," + country.replace("'","") + " : " + recover + "')"	
		else:
			marker = "L.circle([" + lat + "," + lon + "],{color:'#3c9124',fillColor:'#42cc1b',fillOpacity:0.5,radius:" + str(recoverradius) + "}).addTo(map).bindPopup('" + country.replace("'", "") + " : " + recover + "')"
