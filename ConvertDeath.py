...
for line in datarows:
	...
	if (int(deaths) > 0):
		if (prov != ""):
			marker = "L.circle([" + lat + "," + lon + "],{color:'#6e6f70',fillColor:'#363738',fillOpacity:0.5,radius:" + str(deathsradius) + "}).addTo(map).bindPopup('" + prov.replace("'", "") + "," + country.replace("'","") + " : " + deaths + "')"	
		else:
			marker = "L.circle([" + lat + "," + lon + "],{color:'#6e6f70',fillColor:'#363738',fillOpacity:0.5,radius:" + str(deathsradius) + "}).addTo(map).bindPopup('" + country.replace("'", "") + " : " + deaths + "')"
